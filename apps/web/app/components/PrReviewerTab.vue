...
<template>
  <div class="flex flex-1 overflow-hidden">
    ...
    >
      ...
      >
        <span class="text-sm font-medium text-[#ccc]">PR Review</span>
        <span
          ...
          <button
            ...
            :class="
              prReviewer.selectedRepos.value.has(repo.label)
                ...
                : 'bg-[rgb(255_255_255/4%)] text-[#888] hover:text-[#ccc]'
            "
      ...
      >
        ...
        <div class="flex items-center gap-1">
          ...
          <select
            ...
            class="ml-auto min-w-0 truncate rounded-md border border-[rgb(255_255_255/8%)] bg-[rgb(255_255_255/4%)] px-2 py-1 text-xs text-[#ccc] outline-none focus:border-[rgb(255_255_255/8%)]"
          >
      ...
      >
        <span class="text-xs font-medium text-[#a78bfa]">
          ...
          >
            ...
            >
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-0.5 text-xs text-[#444]">by @{{ pr.author }}</p>
              </div>
            </div>
          </template>
        </div>
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      >
        ...
        <div
          v-if="prReviewer.reviewHistory.reviews.value.length === 0"
          class="px-4 pb-3 text-center text-xs text-[#444]"
        >
          今天還沒有 Review 紀錄
        </div>
        ...
        <template v-else>
          <div class="max-h-40 overflow-y-auto px-4 pb-3">
            ...
            >
              <span class="shrink-0 font-mono text-[#8b5cf6]"
                >{{ review.repoLabel }}#{{ review.prNumber }}</span
              >
              <span class="min-w-0 truncate text-[#888]">{{
                review.prTitle
              }}</span>
              <div class="ml-auto flex shrink-0 items-center gap-1.5">
                ...
                <span v-if="review.majors > 0" class="text-yellow-400"
                  >🟡{{ review.majors }}</span
                >
                <span v-if="review.minors > 0" class="text-[#22c55e]"
                  >🟢{{ review.minors }}</span
                >
                <span v-if="review.suggestions > 0" class="text-[#8b5cf6]"
          ...
          <!-- Stats bar -->
          <div
            class="flex items-center gap-4 border-t border-[rgb(255_255_255/6%)]/50 px-4 py-2 text-xs text-[#888]"
          >
    ...
    <UModal v-model:open="showSlackModal">
      <template #content>
        <div class="p-6">
          <h3 class="mb-4 text-sm font-semibold text-[#fafafa]">
            發送今日 Review 報告到 Slack
          </h3>
          <div class="mb-4">
            <label class="mb-1 block text-xs text-[#888]">Channel 名稱</label>
            <input
              v-model="slackChannel"
              class="w-full rounded-lg border border-[rgb(255_255_255/8%)] bg-[rgb(255_255_255/4%)] px-3 py-2 text-sm text-[#ccc] placeholder-[#444] focus:border-[#a78bfa] focus:outline-none"
