...
type: feedback
company: kkday            # Company scope (omit for workspace-wide memories)
trigger_count: 1          # Number of times referenced/applied (= 1 when first created)
last_triggered: 2026-03-30  # Date last referenced
---
```
...
Based on the semantic content of the feedback, find the most appropriate file in `.claude/rules/kkday/`:

| Feedback Topic | Target File |
|----------------|-------------|
| Sub-agent delegation behavior | `rules/kkday/sub-agent-delegation.md` |
| PR / Review workflow | `rules/kkday/pr-and-review.md` |
| JIRA conventions | `rules/kkday/jira-conventions.md` |
| Skill usage | `rules/kkday/skill-routing.md` |
...
Target: {rules/kkday/xxx.md} § {section}
Content to add:
  {drafted rule text}
...
- **When saving a memory** that is specific to one company's workflow, codebase, or conventions → include `company: kkday` in frontmatter
